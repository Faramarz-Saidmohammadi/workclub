...
const describeWithMongo = process.env.TEST_MONGO_URI || process.env.USE_IN_MEMORY_MONGO === "true"
  ? describe
  : describe.skip;
...
describeWithMongo("WorkClub API integration", () => {
  ...
  it("refreshes the correct user in a multi-user database and rejects concurrent replay", async () => {
    ...
    const second = await registerOwner(app, "second@integration.test", "Second Studio");
    expect(second.refreshCookie).toContain("workclub_refresh=");

    const [firstRefresh, replay] = await Promise.all([
      request(app).post("/api/auth/refresh").set("Cookie", second.refreshCookie),
      request(app).post("/api/auth/refresh").set("Cookie", second.refreshCookie),
    ]);
  ...
  it("enforces tenant and Member assignment boundaries at the API layer", async () => {
    ...
    expect(await Proposal.countDocuments({ workspace: first.user.workspace })).toBe(0);
  });
  ...
  it("prevents concurrent invoices from consuming the same time entry twice", async () => {
    ...
    expect(releasedBillable.body.result.map((item: { _id: string }) => item._id)).toContain(
      entry._id.toString(),
    );
    const basePayload = {
...
async function registerOwner(
  ...
  return { user, token: response.body.result.accessToken as string, refreshCookie };
}
